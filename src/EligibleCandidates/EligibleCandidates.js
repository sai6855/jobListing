...
const EligibleCandidates = ({location:{details,role}}) => {
    ...
    return (
        <div className='eligible-candidates'>
            ...
            {details===undefined|| details.length===0 ? <h1>No eligible candidates</h1>:(
                <Fragment>
...
<div key={i} className="candidate-details">
    ...
    <h4 className='detail'>Notice period : {detail.noticePeriod}</h4>
    <h4 className='detail'>Salary Asked : {detail.salaryAsked}</h4>
    <strong className='detail '>Technologies : {detail.technologies.map(tech=>tech.label)} </strong>
</div>
    )
})}
